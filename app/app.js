...
function reviewEmail(text, inboxItem, file){
  ...
  ACT.addFromEmail = async () => {
    ...
    if (trip && Array.isArray(trip.hidden)) { const need = kind === 'flight' ? 'flight' : kind === 'stay' ? 'stay' : kind === 'reservation' ? '' : 'food'; if (need && trip.hidden.includes(need)) { trip.hidden = trip.hidden.filter(k => k !== need); await Store.put('trips', trip); } }
    ...
    await Store.put('inbox', Object.assign({ id: Store.uid(), at: Date.now() }, inboxItem || {}, { title: kind === 'flight' ? 'Flight ' + legs.map(l => l.from + '→' + l.to).join(', ') + (legs[0] && legs[0].no ? ' · ' + legs[0].no.split(' ')[0] : '') : r.title, kind, tripCity: trip ? trip.city : '', tripId: trip ? trip.id : '', dates: kind === 'flight' ? legs.map(l => l.date).filter(Boolean) : [r.date || today()], amount: r.amount || 0, status: 'Added', raw: '' }));
