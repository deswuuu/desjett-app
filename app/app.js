...
function bind(root){
  ...
  root.querySelectorAll('[data-act]').forEach(el => { const ev = el.dataset.on || 'click'; el['on' + ev] = e => { const [n, arg] = el.dataset.act.split('|'); if (ACT[n]) ACT[n](arg, el, e); }; });
...
function flightInfo(m){ const t = m.tripId ? D.trip(m.tripId) : null; const all = (t ? D.tripMoments(t.id) : D.moments()).filter(x => x.flight); const first = all.map(x => x.date).sort()[0]; const flyer = D.user(m.flight.who || (t && t.flyer) || (m.cost && m.cost.paidBy) || m.authorId) || {}; const who = flyer.name || ''; const same = (a, b) => a && b && a.toLowerCase().split(/[ ,]/)[0] === b.toLowerCase().split(/[ ,]/)[0]; const dest = AIRPORTS[m.flight.to] || ''; const out = same(dest, flyer.city) ? false : D.users().some(u => u.id !== flyer.id && same(dest, u.city)) || (t && same(dest, t.city)) ? true : m.date === first; return { out, who, time: (out ? m.flight.arr : m.flight.dep) || '', text: `${who} ${out ? 'lands' : 'flies home'}` }; }
function flightLabel(m){ const f = flightInfo(m); return `${f.text} ${f.time}`.trim(); }
function renderDays(app){
  const t = D.trip(route.id); const ms = D.tripMoments(t.id); const n = daysBetween(t.start, t.end); const firstFlight = (ms.filter(m => m.flight).map(m => m.date).sort()[0]) || '';
  let rows = '';
  for (let i = 0; i <= n; i++) { const d = addDays(t.start, i); const dm = ms.filter(m => m.date === d); const plan = dm.find(m => m.kind === 'plan'), fls = dm.filter(m => m.flight), fl = fls[0]; const out = fl && fl.date === firstFlight; const title = plan ? (plan.title||plan.text) : fl ? flightLabel(out ? fls[fls.length-1] : fl) : dm.find(m => m.text) ? dm.find(m => m.text).text : ''; const others = dm.filter(m => m.kind !== 'booking').length; rows += `<button class="day" data-go="day/${t.id}_${d}"><div class="d">${parseDate(d).getDate()}</div><div class="w ${title?'':'open'}">${title ? esc(title) : 'Open'}${others ? `<span>${others} moment${others>1?'s':''}</span>` : plan && plan.note ? `<span>${esc(plan.note)}</span>` : ''}</div></button>`; }
...
function momentSheet(m, editing){
  ...
  ACT.mVoice = async () => { keep(); if (rec) { rec.stop(); return; } try { const stream = await navigator.mediaDevices.getUserMedia({ audio:true }); const chunks = []; const started = Date.now(); rec = new MediaRecorder(stream); rec.ondataavailable = e => chunks.push(e.data); rec.onstop = async () => { stream.getTracks().forEach(t => t.stop()); const blob = new Blob(chunks, { type: rec.mimeType || 'audio/webm' }); rec = null; m.voice = await Store.putBlob(blob, (blob.type.includes('mp4') ? 'm4a' : 'webm')); m.voiceDur = (Date.now() - started) / 1000; redraw(); toast('Voice note saved'); }; rec.start(); redraw(); toast('Recording… tap again to stop'); } catch (e) { toast('Microphone not available'); } };
...
// ---------- calendar ----------
let calMonth = null, calSel = null, calOurs = false;
function renderCalendar(app){
  ...
  app.innerHTML = `<div class="screen"><div class="bar"><span style="display:flex;gap:10px;align-items:center"><button data-act="calNav|-1">‹</button><span>${calMonth.y}</span><button data-act="calNav|1">›</button></span><span style="display:flex;gap:8px;align-items:center"><button class="chip ${calOurs?'on':''}" style="font-size:11px" data-act="calOurs">Our plans only</button><button class="l" data-act="importIcs">Import .ics</button></span></div>
    ...
    <div class="agenda">${groups.join('')}</div>
  </div>${nav('calendar')}`;
}
...
function reviewEmail(text, inboxItem){
  const r = parseConfirmation(text); const me = D.me(); let paidBy = me.id, flyer = me.id; let tripId = (route.name === 'emails' && route.id && D.trip(route.id)) ? route.id : (r.trip ? r.trip.id : ''); let ddOpen = false; let legs = (r.legs && r.legs.length ? r.legs : [{ from: r.from||'', to: r.to||'', date: r.date||today(), dep: r.dep||'', arr: r.arr||'', no: r.no||'' }]);
  ...
  ACT.addFromEmail = async () => {
    ...
    if (kind === 'flight') { for (const [i, l] of legs.entries()) { if (!l.from && !l.to) continue; const date = l.date || (trip ? (i ? trip.end : trip.start) : today()); await Store.put('moments', { id: Store.uid(), tripId: trip ? trip.id : '', date, authorId: me.id, kind: 'booking', text: 'Flight ' + l.from + ' → ' + l.to, flight: { no: l.no, from: l.from, to: l.to, dep: l.dep, arr: l.arr, code: r.code || '', who: flyer }, cost: i === 0 && r.amount ? { amount: r.amount, paidBy, tag: 'flight' } : null, createdAt: Date.now() + i }); } if (trip) { const ds = legs.map(l => l.date).filter(Boolean).sort(); let ch = false; if (ds[0] && ds[0] < trip.start) { trip.start = ds[0]; ch = true; } if (ds.length && ds[ds.length-1] > trip.end) { trip.end = ds[ds.length-1]; ch = true; } if (!trip.flyer) { trip.flyer = flyer; ch = true; } if (ch) await Store.put('trips', trip); } }
    else { const m = { id: Store.uid(), tripId: trip ? trip.id : '', date: r.date || today(), authorId: me.id, kind: kind === 'reservation' ? 'plan' : 'booking', text: r.title, createdAt: Date.now(), cost: r.amount ? { amount: r.amount, paidBy, tag: kind === 'stay' ? 'stay' : 'food' } : null }; if (kind === 'reservation') { m.title = r.title; m.time = r.dep || ''; m.who = 'both'; } await Store.put('moments', m); }
